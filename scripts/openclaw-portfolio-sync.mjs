...
const SHEET_ID =
  ...
  "18TTdqThrMOflgqJjnpgSjIeKl_nJZxH2MTF6vg3M-YA";
const POSITIONS_RANGE =
  process.env.PORTFOLIO_POSITIONS_RANGE || "Positions!A1:Z200";
const TRADES_RANGE = process.env.PORTFOLIO_TRADES_RANGE || "Trades!A1:Z500";
const ACCOUNT =
  process.env.PORTFOLIO_GOOGLE_ACCOUNT || "montgomerie.scott@gmail.com";
const BASE = (
  process.env.MISSION_CONTROL_BASE_URL || "http://127.0.0.1:8000"
).replace(/\/$/, "");
const TOKEN = process.env.MISSION_CONTROL_TOKEN || "";
const API_KEY = process.env.MISSION_CONTROL_API_KEY || "";
const BETTER_AUTH_URL = (
  process.env.MISSION_CONTROL_BETTER_AUTH_URL || ""
).replace(/\/+$/, "");
...
async function exchangeKeyForJwt() {
  ...
  if (!res.ok) {
    throw new Error(
      `API key exchange refused (HTTP ${res.status}) — check the key is active and not expired`,
    );
  }
...
async function main() {
  ...
  if (!keyMode && !TOKEN) {
    process.stdout.write(
      JSON.stringify({
        ok: false,
        error:
          "MISSION_CONTROL_TOKEN is not set (or use MISSION_CONTROL_API_KEY + MISSION_CONTROL_BETTER_AUTH_URL)",
      }),
    );
    process.exitCode = 1;
    return;
  ...
  const doPost = (bearer) =>
    fetch(`${BASE}/api/v1/portfolio/review/run`, {
      ...
      body: JSON.stringify({
        positions_rows: positionsRows,
        trades_rows: tradesRows,
      }),
    });
