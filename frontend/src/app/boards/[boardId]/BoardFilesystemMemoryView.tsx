...
import {
  ...
  type FilesystemMemoryContent,
  type FilesystemMemoryFile,
...
const emptyDailyMessage =
  "No daily memory files yet. The lead agent has not written any dated memory logs.";

type BoardFilesystemMemoryViewProps = {
  active: boolean;
  boardId: string;
};

export function BoardFilesystemMemoryView({
  active,
  boardId,
}: BoardFilesystemMemoryViewProps) {
  const [reloadToken, setReloadToken] = useState(0);
  const [overview, setOverview] = useState<FilesystemMemoryOverview | null>(
    null,
  );
  ...
  const [fileCache, setFileCache] = useState<
    Record<string, FilesystemMemoryContent>
  >({});
  ...
  useEffect(() => {
    if (!active || !boardId || overview) return;

    let cancelled = false;
    setIsOverviewLoading(true);
    setOverviewError(null);

    void fetchFilesystemMemoryOverview(boardId)
      .then((data) => {
        if (cancelled) return;
        setOverview(data);
        setSelectedPath(
          (current) => current ?? selectInitialFilesystemMemoryPath(data),
        );
      })
      .catch((error: unknown) => {
        if (cancelled) return;
  ...
  }, [active, boardId, overview, reloadToken]);

  useEffect(() => {
    if (!active || !boardId || !selectedPath || fileCache[selectedPath]) return;

    let cancelled = false;
    setIsSelectedLoading(true);
    setSelectedError(null);

    void fetchFilesystemMemoryFile(boardId, selectedPath)
      .then((file) => {
        if (cancelled) return;
  ...
  const longTermMatches = useMemo(() => {
    ...
    return matchesFilesystemMemorySearch(
      longTermMemory,
      searchTerm,
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="w-full max-w-sm">
            <label
              htmlFor="filesystem-memory-search"
              className="mb-2 block text-xs font-semibold uppercase tracking-wider text-slate-500"
            >
              Search loaded memory
            </label>
            <Input
              id="filesystem-memory-search"
              value={searchTerm}
              ...
              <div className="px-5 py-4">
                {!longTermMemory ? (
                  <p className="text-sm text-slate-500">
                    `MEMORY.md` is not available in the lead workspace yet.
                  </p>
                ) : !longTermMatches ? (
                  <p className="text-sm text-slate-500">
              ...
              <div className="max-h-[720px] overflow-y-auto p-3">
                {filteredDailyFiles.length === 0 ? (
                  ...
                  <div className="space-y-1.5">
                    {filteredDailyFiles.map((file: FilesystemMemoryFile) => (
                      <button
                        key={file.path}
                        type="button"
