...
server.tool("portfolio_list_positions", "List portfolio positions from Mission Control, with optional ticker, rationale, and flag filters.", listPositionsInputSchema, async (input) => {
    try {
...
server.tool("portfolio_get_position", "Fetch full portfolio position detail, including current rationale and latest linked review summary.", getPositionInputSchema, async (input) => {
    try {
...
server.tool("portfolio_save_rationale", "Create or update durable trade rationale for a Mission Control portfolio position.", saveRationaleInputSchema, async (input) => {
    try {
...
server.tool("portfolio_list_reviews", "List portfolio reviews, optionally narrowed to one position key and capped by limit.", listReviewsInputSchema, async (input) => {
    try {
